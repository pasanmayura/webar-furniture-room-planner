...
export function loadTablePreview() {
  const canvas = document.getElementById("tablePreviewCanvas");
  ...
  const animationFrame = () => {
    ...
    tablePreview.animationFrame = requestAnimationFrame(animationFrame);
  };

  tablePreview = { renderer, resize, animationFrame: 0 };
  ...
  new GLTFLoader().load(
    ...
    undefined,
    (error) => {
      console.error("Unable to load table model:", error);
    }
  );
}

export function destroyTablePreview() {
  if (!tablePreview) {
    return;
  }

  window.removeEventListener("resize", tablePreview.resize);
  cancelAnimationFrame(tablePreview.animationFrame);
  tablePreview.renderer.dispose();
  tablePreview = null;
}
