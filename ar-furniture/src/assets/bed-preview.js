...
export function loadBedPreview() {
  ...
  const animationFrame = () => {
    ...
    bedPreview.animationFrame = requestAnimationFrame(animationFrame);
  };

  bedPreview = { renderer, resize, animationFrame: 0 };
...
export function destroyBedPreview() {
  if (!bedPreview) {
    return;
  }

  window.removeEventListener("resize", bedPreview.resize);
  cancelAnimationFrame(bedPreview.animationFrame);
  bedPreview.renderer.dispose();
  bedPreview = null;
}
